...
module stageJS
{
    ...
    export class Texture
    {
        ...
        public uploadFromBitmapData(source:any, miplevel: number /* uint */ = 0): void
        {
            ...
            {
                this.uploadFromImage(<stageJS.BitmapData>source.imageData,miplevel);
            }else
            {
        ...
        public uploadFromImage(source: any, miplevel: number /* uint */ = 0): void
        {
            ...
            Context3D.GL.texParameteri(Context3D.GL.TEXTURE_2D, Context3D.GL.TEXTURE_MAG_FILTER,Context3D.GL.LINEAR  ); //Context3D.GL.NEAREST
            if (this._streamingLevels == 0)
            {
                Context3D.GL.texParameteri(Context3D.GL.TEXTURE_2D, Context3D.GL.TEXTURE_MIN_FILTER,Context3D.GL.LINEAR);// Context3D.GL.NEAREST
            } else {
