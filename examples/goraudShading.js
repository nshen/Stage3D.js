...
(function (test) {
    ...
    (function (goraudShading) {
        ...
        var nMatrix = new stageJS.geom.Matrix3D(); // The normal matrix
        var pMatrix = new stageJS.geom.PerspectiveMatrix3D(); // The projection matrix
        var indexBuffer;
        var vertexBuffer;
        var normalBuffer;
        /**
        ...
        var Geometry = (function () {
            function Geometry() {
                this.vertices = [];
                this.verticesFromFile = []; // del ? 跟上边的一样
                this.uvs = [[]];
                this.colors = [];
                this.normals = [];
                this.indices = [];
            ...
            Geometry.prototype.indexFromFaces = function () {
            ...
            Geometry.prototype.calculateNormals = function (vs, ind) {
            ...
            Geometry.prototype.normalsFromFaces = function (faces) {
                var faceI;
                var normalVectors = [];
            ...
            Geometry.fromThreeJSON = function (json) {
                ...
                var nVertices; //每个面的顶点数，3
                var materialIndex; //还没实现
                var uvIndex; //还没用到
                var normalIndex;
                var colorIndex;
                var face;
                var normal;
                var type; //每个face的第一个数字表示类型
                var isQuad;
                var hasMaterial;
                var hasFaceUv;
                var hasFaceVertexUv;
                ...
                while (offset < zLength) {
                    ...
                    if (hasFaceNormal) {
                        ...
                        normal = new stageJS.geom.Vector3D(normals[normalIndex++], normals[normalIndex++], normals[normalIndex]);
        ...
        var Face = (function () {
            function Face(a, b, c, normal, materials) {
        ...
        function loadModel(url) {
            ...
            $.getJSON(url, function (data) {
                ...
                var indices = getIndicesFromFaces(faces);
                var normals = getNormalsFromFaces(faces); //calculateNormals(vertices, indices);  //导入的normals貌似不正常 //
        ...
        function getNormalsFromFaces(faces) {
            var faceI;
            var normalVectors = [];
            for (var i = 0; i < faces.length; i++) {
                faceI = faces[i];
        ...
        function calculateNormals(vs, ind) {
            //indices have to be completely defined NO TRIANGLE_STRIP only TRIANGLES
            var ns = new Array(vs.length); //normals array
            for (var i = 0; i < vs.length; i++)
                ns[i] = 0;
            var v1x;
            var v1y;
