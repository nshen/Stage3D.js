...
module test.goraudShading
{
    ...
    var vertexBuffer: stageJS.VertexBuffer3D;
    var normalBuffer: stageJS.VertexBuffer3D;
    ...
    function uploadBuffers(numVertices, vertices, normals, indices)
    {
        ...
        normalBuffer = context3d.createVertexBuffer(numVertices, 3);
        normalBuffer.uploadFromVector(normals, 0, numVertices);

        indexBuffer = context3d.createIndexBuffer(indices.length);
        indexBuffer.uploadFromVector(indices, 0, indices.length);

        context3d.setVertexBufferAt("aVertexPosition", vertexBuffer, 0, stageJS.Context3DVertexBufferFormat.FLOAT_3);
        context3d.setVertexBufferAt("aVertexNormal", normalBuffer, 0, stageJS.Context3DVertexBufferFormat.FLOAT_3);
    ...
    function initLights() {
        context3d.setProgramConstantsFromVector("uLightDirection", [-0.25, -0.25, -0.25]);

        if (ambientColor && ambientColor.length ==3) //read from json file
        {
            context3d.setProgramConstantsFromVector("uLightAmbient", [ambientColor[0], ambientColor[1], ambientColor[2], 1.0]);
        }else
        {

            context3d.setProgramConstantsFromVector("uLightAmbient", [0.03, 0.03, 0.03, 1.0]);
        }

        if(diffuseColor && diffuseColor.length == 3)
        {
            context3d.setProgramConstantsFromVector("uLightDiffuse", [diffuseColor[0], diffuseColor[1], diffuseColor[2], 1.0]);
        }else
        {
            context3d.setProgramConstantsFromVector("uLightDiffuse", [1.0, 1.0, 1.0, 1.0]);
        }


        context3d.setProgramConstantsFromVector("uLightSpecular", [1.0, 1.0, 1.0, 1.0]);
        //gl.uniform1f(prg.uShininess, 10.0); //TODO: set float uniform
        context3d.setProgramConstantsFromVector("uMaterialAmbient", [1.0, 1.0, 1.0, 1.0]);
        context3d.setProgramConstantsFromVector("uMaterialDiffuse", [46 / 256, 99 / 256, 191 / 256, 1.0]);
        context3d.setProgramConstantsFromVector("uMaterialSpecular", [1.0, 1.0, 1.0, 1.0]);
    ...
    function onEnterFrame():void
    {
        ...
        context3d.clear(0.3,0.3,0.3,1.0,100);
        ++angle;
        mvMatrix.identity();
...
//////////////////////////////////////////////
    class Geometry
    {
        ...
        public verticesFromFile:number[] = []; // del ? 跟上边的一样
        ...
        public faces:Face[] = [];
        public faceUvs:number[][] = [[]];
        public faceVertexUvs:number[][] = [[]];
        public textureCoordinates:number[] = [];
        public materials:number[] = [];


        public indexFromFaces():void
        {
        ...
        public calculateNormals(vs:number[] , ind:number[]):number[]
        {
            //indices have to be completely defined NO TRIANGLE_STRIP only TRIANGLES
            this.normals = new Array(vs.length); //normals array
            ...
            var ny:number;
            var nz:number;
            for(var i:number = 0 ; i < ind.length ; i+=3)
            {
                //p2 - p1
                v1x = vs[3*ind[i+2]+0] - vs[3*ind[i+1]+0];
                v1y = vs[3*ind[i+2]+1] - vs[3*ind[i+1]+1];
                v1z = vs[3*ind[i+2]+2] - vs[3*ind[i+1]+2];

                //p0 - p1
                v2x = vs[3*ind[i+0]+0] - vs[3*ind[i+1]+0];
                v2y = vs[3*ind[i+0]+1] - vs[3*ind[i+1]+1];
                v2z = vs[3*ind[i+0]+2] - vs[3*ind[i+1]+2];

                //cross product to get the normal of triangle[p0,p1,p2]
                nx = v1y*v2z - v1z*v2y;
                ny = v1z*v2x - v1x*v2z;
                nz = v1x*v2y - v1y*v2x;
                ...
                for(var j:number = 0;j < 3; j++)
                {
                    ...
                    var base:number = 3*ind[i+j]
                    this.normals[base + 0] += nx;
            ...
            for(var i:number = 0;i < vs.length;i+=3)
        ...
        public normalsFromFaces(faces:Face[]):number[]
        {
            var faceI:Face;
            var normalVectors:stageJS.geom.Vector3D[] = [];
            for (var i = 0;i < faces.length; i++)
            {
                faceI = faces[i];
                //有face normal，但没有vertex normal，复制face normal到各顶点
                if(faceI.normal != undefined && faceI.vertexNormals.length == 0)
                {
                    faceI.vertexNormals[0] = faceI.normal.clone();
                    faceI.vertexNormals[1] = faceI.normal.clone();
                    faceI.vertexNormals[2] = faceI.normal.clone();
                }


                if(!normalVectors[faceI.a])
                    normalVectors[faceI.a] = faceI.vertexNormals[0].clone();
                else
                    normalVectors[faceI.a].incrementBy(faceI.vertexNormals[0]);

                if(!normalVectors[faceI.b])
                    normalVectors[faceI.b] = faceI.vertexNormals[1].clone();
                else
                    normalVectors[faceI.b].incrementBy(faceI.vertexNormals[1]);

                if(!normalVectors[faceI.c])
                    normalVectors[faceI.c] = faceI.vertexNormals[2].clone();
                else
                    normalVectors[faceI.c].incrementBy(faceI.vertexNormals[2]);
            ...
            for(var j = 0 ; j < normalVectors.length ; j ++)
            {
                ...
                this.normals.push(normalVectors[j].x);
                this.normals.push(normalVectors[j].y);
                this.normals.push(normalVectors[j].z);
            }
            //console.log("normals:",normals);
            return this.normals;
        }

        public  static fromThreeJSON(json:any):Geometry
        {
            var geometry = new Geometry();

            if ( json.metadata === undefined ||
                json.metadata.formatVersion === undefined ||
                json.metadata.formatVersion !== 3.1 ) {

                console.error( 'Deprecated file format.' );
                return;
            }
            geometry.verticesFromFile = json.vertices;
            ...
            var nVertices:number;//每个面的顶点数，3
            var materialIndex:number;//还没实现
            var uvIndex:number; //还没用到
            var normalIndex:number;
            var colorIndex:number;

            var face:Face;
            var normal:stageJS.geom.Vector3D;

            var type:number;//每个face的第一个数字表示类型
            var isQuad:boolean;
            var hasMaterial:boolean;
            var hasFaceUv:boolean;
            var hasFaceVertexUv:boolean;
            var hasFaceNormal:boolean;
            var hasFaceVertexNormal:boolean;
            var hasFaceColor:boolean;
            var hasFaceVertexColor:boolean;


            var offset:number = 0;
            var zLength:number = faces.length;
            while (offset < zLength)
            {
                ...
                if(hasMaterial)
                {
                    ...
                    face.materialIndex = materialIndex; //与网站不同
                }
                ...
                if ( hasFaceNormal )
                {
                    normalIndex = faces[offset++] * 3; //3个点？
                    normal = new stageJS.geom.Vector3D(
                        normals[normalIndex++] ,
                        normals[normalIndex++],
                        normals[normalIndex]);
                    //vec3.fromValues(normals[ normalIndex ++ ],normals[ normalIndex ++ ],normals[ normalIndex ]);
                    face.normal = normal;
            ...
            geometry.materials = json.materials;
            geometry.indexFromFaces(); //从faces中读出index

            if(geometry.vertices.length == 0)
                geometry.vertices = json.vertices;

            if(json.normals.length > 0)
                geometry.normalsFromFaces(geometry.faces);
            else
                geometry.calculateNormals(geometry.vertices ,geometry.indices);
    ...
    class Face
    {
        ...
        public c:number;
        public d:number;
        public normal:stageJS.geom.Vector3D;
        public colorIndex:number;
        public vertexNormals:stageJS.geom.Vector3D[];
        public vertexColors:number[];
        //public materials:number[];
        public materialIndex:number = 0;
        public constructor(a?,b?,c?,normal?,materials?)
        {
            // vertex indices
            this.a = a;
            this.b = b;
    ...
    function parseThreeJSON(json:any):Face[]
    {
        if ( json.metadata === undefined ||
             json.metadata.formatVersion === undefined ||
             json.metadata.formatVersion !== 3.1 ) {
    ...
    function getIndicesFromFaces(faces:Face[]):number[]
    {
        var indices:number[] = [];
        for(var i = 0;i < faces.length;i++){
    ...
    function getNormalsFromFaces(faces:Face[]):number[]
    {
        var faceI:Face;
        var normalVectors:stageJS.geom.Vector3D[] = [];
        for (var i = 0;i < faces.length; i++)
        ...
        {
            normalVectors[j].normalize();
            normals.push(normalVectors[j].x);
            normals.push(normalVectors[j].y);
            normals.push(normalVectors[j].z);
        }
        //console.log("normals:",normals);
        return normals;
    }
...
/////////////////////////////////////////////////
     ...
     * @returns {number[]}
     */
    function calculateNormals(vs:number[] , ind:number[]):number[]
    {
        //indices have to be completely defined NO TRIANGLE_STRIP only TRIANGLES
        var ns:number[] = new Array(vs.length); //normals array
        for (var i:number = 0; i < vs.length; i++)
            ns[i] = 0;
